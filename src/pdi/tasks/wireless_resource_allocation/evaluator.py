...
from pdi.tasks import TASK_REGISTRY
from pdi.tasks.base import BaseTask
from pdi.datasets.normalizer import Normalizer
from pdi.datasets.wra.utils import (
    compute_ergodic_rates_batched,
    jains_fairness_index,
    clamp_power,
    compute_violation_rate,
)
from pdi.datasets.wra.configs import dataset_name_to_alias
from pdi.datasets.wra.channel_factory import (
    ...
    **kwargs
    ...
    def _visualize_selector_rate_correlation(
        ...
        for (dataset_name, network_id) in selected_keys:
            ...
            if num_nodes <= 1:
                continue
            sel = hard_freq_np[:num_nodes]
            real = real_rates_mean[:num_nodes]
            gen = gen_rates_mean[:num_nodes]
    ...
    def _load_precomputed_h_samples(
        ...
        *,
        ...
        tile_h = available < required

        # Use contiguous temporal indices to preserve channel correlation.
        # When tiling, we select all available indices; the array is tiled
        # after loading so that sequential channel windows wrap around.
        selected_indices = np.arange(min(available, required), dtype=np.int64)
        ...
        if self.h_io_mode == "auto":
            ...
            try:
                ...
                h_mnt = np.transpose(selected, (1, 2, 0))
                if tile_h:
                    h_mnt = self._tile_h_to_required(h_mnt, required, dataset_name, network_id)
                return h_mnt
        ...
        h_mnt = self._load_selected_h_from_timeslots(
            timeslot_files=timeslot_files,
    ...
    @staticmethod
    def _tile_h_to_required(
        h_mnt: np.ndarray,
        required: int,
        dataset_name: str,
        network_id: int,
    ) -> np.ndarray:
        """Tile H along the time axis (last dim) to reach *required* timesteps.

        When the precomputed dataset has fewer channel realizations than
        ``eval_num_realizations``, we repeat (tile) the available realizations
        cyclically.  Power indices are already sampled with replacement, so
        repeating channels simply extends the time-sharing window using the
        same channel statistics.
        """
        available = h_mnt.shape[2]
        if available >= required:
            return h_mnt
        reps = int(np.ceil(required / available))
        h_tiled = np.tile(h_mnt, (1, 1, reps))[:, :, :required]
        logger.info(
            "Tiled H for (%s, %s): %d available -> %d required (%.1fx repeat).",
            dataset_name, network_id, available, required, required / available,
        )
        return np.ascontiguousarray(h_tiled)
