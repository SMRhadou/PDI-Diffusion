...
class ConditionalTemporalMixerEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if output_mode == 'time_varying' and time_varying_method != 'learned_projection':
            raise ValueError(
                ...
                "Expected 'learned_projection'."
            )
        if output_mode == 'time_varying':
            if projection_source_timesteps is None or projection_target_timesteps is None:
                raise ValueError(
                    ...
                    "must be set when output_mode='time_varying'."
                )
        ...
        if self.output_mode == 'time_varying':
    ...
    def forward(
        ...
        if target_timesteps is None:
            raise ValueError(
                "target_timesteps must be provided when output_mode='time_varying'."
            )
        if self.projection_source_timesteps is None or self.projection_target_timesteps is None:
            raise ValueError(
                "projection_source_timesteps and projection_target_timesteps must be set "
                "when output_mode='time_varying'."
            )
