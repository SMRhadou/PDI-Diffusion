...
from pdi.models.components.graph_conv import (
    ResidualGNNBlock,
    GNN,
    _pack_active_nodes_4d_from_indices,
    _unpack_active_nodes_4d,
)
from pdi.models.components.pooling import StridedGraphMaxPool
from pdi.models.components.embeddings import (
    TimeEmbedding,
...
class EmbeddingConfig:
    ...
    cond_temporal_time_varying_method: Literal['learned_projection'] = 'learned_projection'
...
class TemporalNodeCrossAttention(nn.Module):
    ...
    Notes:
    - This module expects query/key/value temporal lengths to already match.
      In UGNN time-varying conditioning, this is satisfied because the
      conditioning path is pre-projected to target horizon T_out before fusion.
    ...
    def forward(
        ...
        if num_active > 0:
            ...
            q_active = q_bn[active_bn]    # (A, T, C)
            kv_active = kv_bn[active_bn]  # (A, T, C)

            qh = self._to_heads(self.q_proj(q_active))   # (A, H, T, D)
            kh = self._to_heads(self.k_proj(kv_active))  # (A, H, T, D)
            vh = self._to_heads(self.v_proj(kv_active))  # (A, H, T, D)
            ...
            attn = self.out_proj(self._from_heads(attn))  # (A, T, C)

            # Scatter back to full (B*N, T, C); inactive rows remain zero.
    ...
    def forward(
        ...
        if self.has_conditioning:
            ...
            if cond_feats.dim() == 3:
                ...
                # Time-varying conditioning: must match the main signal horizon.
                if cond_feats.size(1) != T:
                    raise ValueError(
                        f"Time-varying conditional embeddings must match T={T}, got {cond_feats.size(1)}"
                    )
                if active_mask is not None:
...
class UGNNDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.output_proj = nn.Linear(final_channels, out_channels)
...
from pdi.models import MODEL_REGISTRY
        ...
        **kwargs, # absorb non-init config fields like `name`
        ...
        if self.num_bottleneck_layers > 0:
            ...
            self.bottleneck = GNN(
                ...
                temporal_dilations=config.gnn_config.temporal_dilations,
