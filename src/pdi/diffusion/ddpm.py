...
class DDPM(BaseDiffusion):
    ...
    def _collect_selector_losses_and_diagnostics(self) -> Optional[torch.Tensor]:
        ...
        if selector_diags:
            ...
            numeric_keys = [
                "temperature", "entropy_mean_norm", "entropy_reg",
    ...
    def training_loss_stratified_t(
        ...
        Returns:
            (scalar_loss, bin_losses):
                scalar_loss  — mean reconstruction loss + selector aux, shape ().
                bin_losses   — dict mapping bin label → list of per-graph float
                               reconstruction losses (aux loss excluded).
        """
        B = data.num_graphs
        N = data.y.size(0) // B
        T_seq = data.y.size(1)
    ...
    def compute_elbo_per_trajectory(
        ...
        **not** a strict variational bound — labelled ``diffusion_nll_proxy``
        ...
        for _ in tqdm.tqdm(range(n_mc_samples), desc="ELBO estimator MC samples", unit="sample"):
            t = torch.randint(0, self.num_timesteps, (B,), device=device, dtype=torch.long)
            noise = torch.randn_like(x0)
            xt = self.add_noise(x0, t, noise)
