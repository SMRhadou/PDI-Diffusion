...
``cond_channels = F_cond_dataset + lambda_channels``.
"""
...
class ScoreNetWithLambda(nn.Module):
    ...
    Args:
        ...
        expected_cond_feats: The dataset-provided number of conditioning
            ...
            emit ``expected_cond_feats + lambda_channels`` channels. If no
            dataset cond exists, set to 0; the wrapper will produce
            lambda-only cond tensor.
        lambda_channels: Number of per-node lambda conditioning channels.
            Default 1 (single lambda per node). Set to 4 for enriched
            portfolio constraints (caller must pre-expand dual_lambda to
            [B, N, 4] before passing).
    """
    ...
    def _augment_cond(
        ...
        dual_lambda: torch.Tensor,  # [B, N] or [B, N, C]
        batch_size: int,
        num_nodes: int,
        device: torch.device,
        dtype: torch.dtype,
        ...
        t_cond = cond.size(1)
        C = lam.shape[-1]
        lam_expanded = lam.expand(batch_size, t_cond, num_nodes, C)
