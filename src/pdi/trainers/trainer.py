...
from pdi.diffusion.base import BaseDiffusion
from pdi.tasks.base import BaseTask
from pdi.trainers.base_trainer import BaseTrainer
from pdi.trainers.best_model_tracker import BestModelTracker
...
class DiffusionTrainer(BaseTrainer):
    ...
    def _should_collect_heavy_selector_diagnostics(self, epoch: int) -> bool:
        """
        ...
        `diagnose_selector_every_n_epochs` semantics:
            - 0: disabled
            - n>0: collect on eval epochs whose 0-based eval index is
              divisible by ``ceil(n / eval_period)``, ensuring alignment
              with the eval schedule.  Falls back to checking against the
              eval schedule directly when ``n`` equals the eval period
              (the common case).
        """
        ...
        if self.max_epochs is not None:
            if not self.eval_schedule.should_eval(epoch, self.max_epochs):
                return False
        # Use the same 1-indexed cadence convention as other periodic knobs.
    ...
    def _plot_selector_sampling_diagnostics(
        ...
        *,
        ...
        if not viz_save_dir:
            return
        try:
            from pdi.utils.plotting import plot_selector_sampling_phase_heatmaps
        except Exception as e:
    ...
    def _plot_epoch_artifacts(
        ...
        *,
        ...
        should_plot_epoch_summaries = (
            (epoch + 1) % plot_epoch_summaries_every_n_epochs == 0
        )
        ...
        if self.diagnose_model_enabled:
            ...
            ) and selector_diag_summary_path is not None and self._should_plot_on_epoch(
                epoch=epoch,
            ...
            if selector_network_node_stats:
                ...
                ) and len(capped_node_stats) > 0 and self._should_plot_on_epoch(
                    epoch=epoch,
                ...
                ) and len(capped_hard_stats) > 0 and self._should_plot_on_epoch(
                    epoch=epoch,
        ...
        os.makedirs(self.save_dir, exist_ok=True)
        try:
            from pdi.utils.plotting import plot_epoch_summaries_jsonl
            ...
            if self.diagnose_model_enabled and (
                ...
                from pdi.utils.plotting import (
    ...
    def fit(self, train_loader: DataLoader, val_loader: Optional[DataLoader] = None,
        ...
        for epoch in range(self.max_epochs):
            ...
            collect_heavy_selector_diags = self._should_collect_heavy_selector_diagnostics(epoch)
            ...
            if avg_selector_diags:
                log.info(
                    "Avg Selector Diagnostics: temp=%.4f ratio=%.4f ent_reg=%.4f aux=%.6f levels=%.2f",
    ...
    def evaluate(
        ...
        try:
            for data in tqdm(loader, desc=f"Evaluating on {eval_split} set", unit="batch", total=max_batches):
                    data = data.to(self.device)

                    # Stratified-t loss: same scalar, but also fills per-bin stats.
                    with autocast_ctx:
                        if hasattr(self.diffusion, "training_loss_stratified_t"):
                            loss, bin_losses = self.diffusion.training_loss_stratified_t(data)
    ...
    def extract_dataset_info(self, dataset: Union[Dataset, Subset]) -> dict:
        ...
        from pdi.datasets.replicated_dataset import ReplicatedDataset
