...
def plot_epoch_summaries_jsonl(
    ...
    _, task_return_corr = _extract_series(rows, ["val_return_correlation", "val_task_return_correlation", "task_return_correlation"])
    ...
    _, train_val_return_corr = _extract_series(rows, ["train-val_return_correlation", "train-val_task_return_correlation"])
    ...
    # 5c) Return Correlation (Tier 1) — val & train-val
...
def plot_selector_sampling_phase_heatmaps(
    ...
    if dataset_tag is not None:
        filename = (
            ...
            f"_epoch_{epoch + 1:04d}.{fmt}"
        )
    else:
        filename = f"selector_sampling_heatmap_{split_tag}_n{network_tag}_epoch_{epoch + 1:04d}.{fmt}"
    plt.savefig(out / filename, dpi=150)
    plt.close()
