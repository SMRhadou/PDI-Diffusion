...
from pdi.datasets import (
    discover_datasets,
    get_dataset_builder,
    DATASET_REGISTRY,
)

from pdi.tasks import (
    discover_tasks,
    TASK_REGISTRY,
)
...
from pdi.trainers import DiffusionTrainer
...
def _inject_sp500_destandardization(
    ...
    try:
        ...
        if hasattr(task, "set_target_destandardization"):
            from pdi.datasets.sp500.dataset import SP500Stocks
...
def _init_wandb(cfg: DictConfig, hydra_cfg):
    ...
    from pdi.utils.wandb_context import (
        build_wandb_context,
        merge_wandb_context,
    )
    ...
    return wandb.init(
        project=cfg.wandb.get("project", "Graph-Signal-Generative-Diffusion-Modeling"),
